...
import { cases, caseEvents, invoices, contacts, documents, timeEntries } from "@/database/schema";
...
export async function getDashboardStats(firmId: string): Promise<DashboardStats> {
  ...
  const [
    ...
    db
      .select({ count: count() })
      .from(cases)
      .where(and(eq(cases.firmId, firmId), eq(cases.status, "activo"))),
    db
      .select({ count: count() })
      .from(caseEvents)
    ...
    db
      .select({
        total: sql<string>`COALESCE(SUM(${timeEntries.durationMinutes}), '0')`,
      })
      .from(timeEntries)
  ...
  return {
    ...
    pendingAmount: pendingInvoicesResult?.total ?? "0",
